...
//encontrar planeta
function findPlanet(planet: string) {
  return planets.find((p) => p.name === planet) || null;
}

//mudar situação
function changeSituation(planet: Planet, situation: PlanetSituation) {
...
function choosePlanetSituation() {
  const option = Number(
    prompt(`Qual a situação do planeta:
    1 - Habitado
    2 - Habitavel
    3 - Inabitavel
    4 - Inexplorado`)
...
function firstOption() {
  ...
  let planetCoordinates: PlanetCoordinates = [0, 0, 0, 0];
  for (let i = 0; i < 4; i++) {
    let coordinate = Number(prompt(`Informe a coordenada ${i + 1}: `));
    planetCoordinates[i] = coordinate;
  }

  const planetSituation: PlanetSituation = choosePlanetSituation();
